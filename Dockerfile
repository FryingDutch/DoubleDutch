...
FROM base AS production_no_test
...
COPY --from=production_no_test /usr/local/lib/. /usr/local/lib/
COPY --from=production_no_test /server /config.txt /
RUN ldconfig
...
# Run
FROM production_no_test AS production
